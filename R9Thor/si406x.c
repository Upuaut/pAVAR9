...
static void _set_modem()
{
	...
	uint8_t data[] = { 0x20, 0x01, 0x00, 0xAA };
...
void si_set_frequency(uint32_t freq)
{
	...
	else if(freq < 239000000UL) { outdiv = 16; band = 4; }
	else if(freq < 353000000UL) { outdiv = 12; band = 3; }
	else if(freq < 525000000UL) { outdiv = 8;  band = 2; }
	else if(freq < 705000000UL) { outdiv = 6;  band = 1; }
	else                        { outdiv = 4;  band = 0; }
	...
	{
		uint8_t data[] = { 0x20, 0x01, 0x51, 8 + band };
		_send_command(CMD_SET_PROPERTY, data, sizeof(data));
	}
...
{
	uint8_t data[] = { channel, 0x23, 0x00, 0x00 };
